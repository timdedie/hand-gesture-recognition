...
\textbf{Task.} We built a system that recognizes hand gestures from a webcam feed. It classifies five gestures: thumbs up, thumbs down, peace sign, open palm, and no hand.
...
\end{itemize}
Total: about 2.19 million parameters.
...
\end{itemize}
Each augmentation is applied with 50\% probability during training.
...
\section*{Results}
\textbf{Augmentation results.} Table~\ref{tab:augmentation} shows that flip+rotate gives the best results -- even better than using all augmentations together.
...
\textbf{Dataset size results.} Table~\ref{tab:datasize} shows augmentation helps a lot when you have little data (90\% $\rightarrow$ 98\% with only 50 samples). With enough data, augmentation matters less.
...
\textbf{Confusion matrix.} Figure~\ref{fig:conf_matrix} shows the confusion matrix for our best model. No misclassifications.
