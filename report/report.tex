...
\textbf{Contributions.} Tim designed and implemented the CNN architecture, wrote the training and evaluation scripts, and developed the real-time webcam demo with MediaPipe integration. Nick handled data collection by setting up the webcam capture workflow, built the preprocessing pipeline with custom augmentation functions, and ran experiments to determine which augmentation combinations worked best.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.1\columnwidth]{images/training_curves.png}
...
\textbf{Future work.}
\begin{itemize}
    \item Add more gesture classes (e.g., pointing, fist, OK sign)
    \item Support two-hand detection for more complex interactions
    \item Train on a larger dataset with multiple people to improve generalization
    \item Optimize for mobile deployment using model quantization
    \item Explore temporal modeling (LSTM/transformer) to recognize gesture sequences
\end{itemize}
